...
const OneNewsPage = () => {
  ...
  const [likeCount, setLikeCount] = useState(currentNews.likes);
  ...
  const handleLikeClick = async () => {
    try {
      if (likePressed) {
        ...
        await axios.put(
          "https://d0a8-78-62-142-17.ngrok-free.app/api/news/" + currentNews.id,
          {
            headers: {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const currentNewsResponse = await axios.get(
          "https://d0a8-78-62-142-17.ngrok-free.app/api/news/" + newsId,
          {
            headers: {
              "Content-Type": "application/json",
              "ngrok-skip-browser-warning": true,
        ...
        setCurrentNews(currentNewsResponse.data);
      } catch {
    ...
    fetchData();
  }, [currentNews, setCurrentNews, newsId]);
      ...
      {!loading && !error && (
        <Section>
          <NewsHeader
            ...
            authorName={currentNews.authorUsername}
            avatarUrl={null}
          />
          <Image
